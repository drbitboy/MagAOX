 ...
 * argv lists prefix paths of FIFO files (named pipes) of Driver programs
     to run or sockets to connect for Devices.
 * Drivers are assumed to be restarted elsewhere (resurrector)
 * Each local Driver arg /opt/MagAOX/drivers/fifos/device.{in|out}
     is assumed to provide INDI traffic via named pipes. Local Drivers'
     stderrs are unused
 * We only support Drivers that advertise support for one Device. The
     problem with multiple Devices in one Driver is without a way to
     know what they _all_ are there is no way to avoid sending all
 ...
 * All newXXX() received from one Client are echoed to all other Clients
     which have shown an interest in the same Device and property.
